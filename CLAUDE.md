...
GET    /api/v1/billing                       Current user's plan/subscription (ACTIVITY_CRUD)
POST   /api/v1/billing/checkout              Upgrade — STUB simulates PRO unless Stripe wired (ACTIVITY_CRUD)
POST   /api/v1/billing/cancel                Downgrade to FREE (ACTIVITY_CRUD)

POST   /api/v1/attachment                    Attach a file/image to an entity, <=5 MB (ACTIVITY_CRUD)
GET    /api/v1/attachment?parentType=&parentId= List an entity's attachments (ACTIVITY_CRUD)
DELETE /api/v1/attachment/{id}               Soft-delete attachment (ACTIVITY_CRUD)

PUT    /api/v1/user/avatar                    Set current user's profile picture, <=5 MB (USER_OPS)

GET    /api/v1/trash?state=DELETED|ARCHIVED   List soft-deleted (default) or archived projects/tasks/reminders as unified TrashItemDTO rows (ACTIVITY_CRUD)
PUT    /api/v1/trash/archive?type=&id=         Archive an active item — hidden from lists, kept in DB (ACTIVITY_CRUD)
PUT    /api/v1/trash/restore?type=&id=         Restore an item to the active list (from trash or archive) (ACTIVITY_CRUD)
DELETE /api/v1/trash?type=&id=                 Permanently delete (purge) an item (ACTIVITY_CRUD)
...
/gantt      → Gantt timeline — task duration bars + reminder milestones + daily activity strip (protected)
/trash      → Trash & Archive — restore / purge soft-deleted + archived projects/tasks/reminders (protected)
...
- [x] **Attachments UI + detail modals + confirm dialogs + PWA install (frontend)** — reusable `Modal` (framer-motion), `ConfirmProvider`/`useConfirm` (replaces every `window.confirm` with a real modal across Activity/Projects/ProjectDetail/Reminders/Profile/Admin), a rich drag-and-drop `FileDropzone`, and an `AttachmentPanel` (image **carousel** + file download list + upload, ≤5 MB). Activity rows are now **rich clickable cards** → a detail modal showing the activity + its **files/images carousel** (uploads go through the attachment API; the create form's single-image picker was removed). `InstallButton` (via `usePwaInstall` on `beforeinstallprompt`) added to the sidebar + landing nav so the PWA is installable from the UI. Visual fixes: **wider full-width pages** (`ui-page` 1600px), **yellow-warmed dark theme** (highlight/cyan radials + warmer base), and the **feature-card icons** now render in light mode (the gradient stop classes were missing the `mtm-` prefix). Bigger project cards.
- [x] **Attachments + profile avatar (backend)** — polymorphic `Attachment` entity (parentType ACTIVITY/TASK/PROJECT/REMINDER + parentId, name, contentType, sizeBytes, `dataBase64` text, owner uid), owner-scoped CRUD at `/api/v1/attachment` (list by parent, ≤5 MB enforced in service + `@Size` on the DTO), 3 service tests. `User.avatarBase64` (text) + `PUT /api/v1/user/avatar` to set the profile picture (≤5 MB). Powers the upcoming multi-file carousel + avatar UI. **100 backend tests green.**
- [x] **Comic redesign — foundation + landing + PWA** — pivoted the whole SPA to a vibrant comic aesthetic (Bangers + Baloo fonts; violet/pink/cyan/yellow palette; thick ink outlines + hard offset shadows; halftone paper bg) driven entirely through the existing CSS-variable tokens, so every page re-skinned at once. **Sidebar** app-shell (animated mobile drawer, Framer Motion) replaces the top nav; a **PublicNav** serves the landing/auth pages. **Framer Motion** page transitions; **anime.js** floating hero stickers. New **animated marketing landing** (`Home.js`) with hero + aurora + marquee, features, how-it-works, and **Free/Pro pricing** (for selling the SaaS). **PWA**: MTM manifest + service worker (installable, offline app-shell). Added `framer-motion` + `animejs`. Test infra fixed: created the real `src/setupTests.js` (the old `src/tests/setupTests.js` was never loaded by CRA) with jest-dom + IntersectionObserver/ResizeObserver/matchMedia polyfills so framer-motion components render under jsdom.
- [x] **Subscription / billing (stubbed)** — `Subscription` entity (owner, `plan` FREE/PRO, provider, externalId), `BillingService` + `/api/v1/billing` (GET current, POST `/checkout`, POST `/cancel`), gated under ACTIVITY_CRUD, 3 service tests. **Payments are stubbed**: with no `STRIPE_SECRET_KEY`, `checkout` *simulates* a PRO upgrade (so the paid-tier UX is testable) and returns `simulated:true`; with a key present it declines to charge and points at the (unimplemented) Stripe integration. To enable real billing: add the Stripe SDK, set `STRIPE_SECRET_KEY` + a price id, and implement a Checkout Session + webhook in `BillingServiceImpl` (replace the simulate branch). Frontend: a **Plan** card on the Profile page (shows Free/Pro, a "payments stubbed" note, Upgrade/Cancel).
...
- [x] **Admin dashboard now renders end-to-end (bugfixes)** — the Admin experience was broken by response-unwrapping mismatches plus a migration-leftover backend 500:
  - **`useAuth`/`Profile`/`Admin` read the wrong wrapper key** — `GenericMessageResponseDTO` serializes as `{ payload, message }` (paginated bodies as `{ results, page, size, totalPages }` via `@JsonProperty`), but the frontend read `data.data` / `payload.content`. Result: `useAuth.roles` was always empty → `isAdmin` false → **Admin link hidden + `AdminRoute` bounced admins**; the Roles/Users tabs rendered empty; and the **Profile page never prefilled**. Fixed all four spots to read `data.payload` (+ `results`/`page`/`totalPages`).
  - **`GET /user/all` 500'd** with `No property '_id' found for type 'User'` — `PaginationConstants.DEFAULT_FIELD_TO_SORT_BY` was still the Mongo `"_id"` (removed in the Postgres migration). Changed the default sort field to `"id"`. Verified live: the endpoint now returns the paginated user list with roles resolved.
  - **`CliRunner` bootstrap-admin hardening** — the seeded `mtm_admin` took its roles only from the role subset *created that run*, so on a DB where the roles already existed but the admin didn't, it would be created with **zero roles**. Now resolves the full seed-role id set from the DB and **self-heals** an existing admin that's missing any seed role. (Bootstrap admin: `mtm_admin` / `mtm_password`, all four roles.)
- [x] **Wider content pages** — `.ui-page` max-width `1600px → 1920px` (+ roomier `≥1280px` side padding) so Activity / Projects / History / Trash (and Dashboard/Gantt) cover more screen; Trash's extra `max-w-4xl` cap removed. Reminders/Admin/Profile keep their own comfortable inner caps.
- [x] **Gantt timeline (frontend, dependency-free)** — new `/gantt` page (sidebar link) plotting **tasks as duration bars** (createdAt → dueDate, clamped to the window; DONE tasks muted + ✓), **reminders as diamond milestones** (at `remindAt`; handled ones muted), and a **daily activity-load strip** (sequential single-hue cells shaded by minutes logged) — all on one shared date axis with weekly ticks and a "today" line. Range selector (3 weeks / 45 days / 3 months), per-mark hover tooltips, and a legend (icon + label, never color-alone). Colours were chosen via the **dataviz** method and **validated with the palette script** for both light and dark surfaces: Task violet (`#7c3aed` light / `#8b5cf6` dark) + Reminder teal (`#0d9488`), all six checks passing (CVD-safe); exposed as theme-aware `--mtm-gantt-task-rgb` / `--mtm-gantt-reminder-rgb` tokens. Fed by existing `listTasks` / `listReminders` / `getActivityStats` (dailyBreakdown) — no backend change.
- [x] **Trash / Archive (full-stack)** — a unified **Archive & Trash** section (`/trash`, new sidebar link) to view + **restore** soft-deleted and archived **projects / tasks / reminders**, or **permanently delete** (purge) them. Backend: `archived` flag added to `BaseModel` (default false; set in `@PrePersist`), active-list finders on the three repos switched to null-safe `@Query`s that exclude archived (`findActiveForOwner` etc., treating legacy null as not-archived); new archived-list / deleted-list / `findByIdAndOwner` (state-agnostic) finders. A single `TrashService` + `TrashController` at `/api/v1/trash` flattens all three types into a type-agnostic `TrashItemDTO { id, type, title, subtitle, archived, lastUpdatedAt }` and exposes `GET ?state=DELETED|ARCHIVED`, `PUT /archive`, `PUT /restore`, `DELETE` (purge), gated under `ACTIVITY_CRUD`; 6 new service tests (**112 backend tests green**). Frontend: `Trash.js` page with Trash/Archive tabs + restore/delete-forever (real confirm on purge); **Archive** buttons added to Projects cards, ProjectDetail task cards, and Reminder rows. *(Activities still hard-delete via orphanRemoval, so they're not in Trash yet — would need activity delete converted to soft-delete first.)*
- [x] **All modals centered via portal** — `Modal` now renders through `createPortal(..., document.body)` so it escapes the sidebar-padded, framer-motion–transformed `<main>` (a transformed ancestor becomes the containing block for `position: fixed`, which was shifting modals off-center). Every modal (detail modals + the `ConfirmProvider` dialog, which builds on `Modal`) is now perfectly centered on the viewport.
- [x] **Drag-and-drop task board (frontend)** — task cards on `/projects/:id` are now **draggable** (native HTML5 DnD, no new dependency) between the To do / In progress / Done columns to change status; the target column highlights (`ring-primary`) on drag-over and the dragged card dims. The move is **optimistic** (local state updates instantly, reconciled with `updateTask` + reload; reverts on error). The ←/→ arrow buttons remain as an accessible fallback. Dragging is disabled while a card's sub-tasks panel is expanded so text selection in the sub-task input isn't hijacked. In `frontend/src/Pages/ProjectDetail.js`.
...
- [x] **`/projects/:id` (and any future nested route) loaded/refreshed directly rendered a blank, inert page — every control dead, incl. the "New task"/"Add task" button.** `frontend/package.json` had `"homepage": "."`, so CRA emitted **relative** asset URLs (`./static/js/main.*.js`). On the app's only two-segment route, `./static/...` resolves against `/projects/` → `/projects/static/...`, which 404s; nginx's SPA fallback then serves `index.html` (HTML) for the bundle request, the browser executes HTML as JS (`Unexpected token '<'`), and **React never mounts**. Single-segment routes (`/activity`, `/projects`) resolve `./` to root so they were unaffected — which is why only project-detail pages broke. Fixed by setting `"homepage": "/"` (absolute `/static/...`). Verified headless: full load of `/projects/:id` mounts and the add-task flow succeeds (POST `/task` 200 → row renders). PR #39.
