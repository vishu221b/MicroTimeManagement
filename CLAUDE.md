...
### Backend
- [ ] `ActivityRecordServiceImpl.makeFromRecordLogRequestDTO` — refactor 130-line nested method; uses a `var ref = new Object() { ... }` hack for lambda capture and a null-`recordDate` "summary" record as an implicit side channel
- [ ] `ActivityRecordServiceImpl.formatDateToCorrectStringValue` — depends on `Optional.toString()` format via regex stripping; replace with `.orElseThrow()`
...
- [ ] N+1 role lookup: `UserServiceImpl.replaceRoleIdsWithNamesForUser` queries role-by-id once per role per authenticated request. Add a bulk `findByIdIn` query or a name cache.
